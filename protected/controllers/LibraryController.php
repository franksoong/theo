...
class LibraryController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Library']))
		{
			...
			//soong: set attributes
			$image=CUploadedFile::getInstance($model,'store_path');
			if($image)
			{
				$newimg = 'image_'.time().'_'.rand(1, 9999).'.'.$image->extensionName;
				//根据时间戳重命名文件名,extensionName是获取文件的扩展名
				$image->saveAs('images/uploads/'.$newimg);
				
				//soong: set attributes
				$model->store_path = 'images/uploads/'.$newimg;
				$model->inner_name = $newimg;
				$model->format     = $image->extensionName;
